...
const char * const BITCOIN_CONF_FILENAME = "coinvest.conf";
const char * const BITCOIN_PID_FILENAME = "coinvestd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "coinvest" is a composite category enabling all CoinVest-related debug output
                if(ptrCategory->count(std::string("coinvest"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CoinVest
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CoinVest
    // Mac: ~/Library/Application Support/CoinVest
    // Unix: ~/.coinvest
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CoinVest";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CoinVest";
#else
    // Unix
    return pathRet / ".coinvest";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty coinvest.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override coinvest.conf
