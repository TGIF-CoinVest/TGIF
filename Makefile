...
pkgdatadir = $(datadir)/coinvestcore
pkgincludedir = $(includedir)/coinvestcore
pkglibdir = $(libdir)/coinvestcore
pkglibexecdir = $(libexecdir)/coinvestcore
...
CONFIG_HEADER = $(top_builddir)/src/config/coinvest-config.h
CONFIG_CLEAN_FILES = libcoinvestconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libcoinvestconsensus.pc.in \
	...
	$(top_srcdir)/src/config/coinvest-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/CoinVest/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/CoinVest/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/CoinVest/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/CoinVest/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = coinvest-cli
BITCOIN_DAEMON_NAME = coinvestd
BITCOIN_GUI_NAME = coinvest-qt
BITCOIN_TX_NAME = coinvest-tx
...
COPYRIGHT_HOLDERS_FINAL = The CoinVest Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = CoinVest Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/CoinVest/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = coinvestcore
PACKAGE_BUGREPORT = https://github.com/coinvestpay/coinvest/issues
PACKAGE_NAME = CoinVest Core
PACKAGE_STRING = CoinVest Core 1.0.0
PACKAGE_TARNAME = coinvestcore
PACKAGE_URL = https://coinvest.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/CoinVest
abs_srcdir = /tmp/CoinVest
abs_top_builddir = /tmp/CoinVest
abs_top_srcdir = /tmp/CoinVest
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/CoinVest/build-aux/install-sh
...
pkgconfig_DATA = libcoinvestconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/coinvest-cli.bash-completion \
	       $(top_srcdir)/contrib/coinvest-tx.bash-completion \
	       $(top_srcdir)/contrib/coinvestd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_coinvest_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_coinvest_coverage.info test_coinvest.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/CoinVest-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/coinvest-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/coinvest-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/coinvest-config.h
$(top_srcdir)/src/config/coinvest-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/coinvest-config.h src/config/stamp-h1
libcoinvestconsensus.pc: $(top_builddir)/config.status $(srcdir)/libcoinvestconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/CoinVest-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CoinVest-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CoinVest-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_coinvest.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_coinvest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_coinvest_filtered.info: test_coinvest.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_coinvest_filtered.info
...
#test_coinvest_coverage.info: baseline_filtered_combined.info test_coinvest_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_coinvest_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_coinvest_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_coinvest_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_coinvest.coverage/.dirstamp:  test_coinvest_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_coinvest.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_coinvest.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
