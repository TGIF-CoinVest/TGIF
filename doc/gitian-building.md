...
*Setup instructions for a Gitian build of CoinVest Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CoinVest
...
- [Building CoinVest Core](#building-coinvest-core)
...
Clone the git repositories for CoinVest Core and Gitian.
...
in the CoinVest Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building CoinVest Core
----------------

To build CoinVest Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the CoinVest Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coinvest/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/coinvestpay/coinvest
...
For example:
```bash
URL=https://github.com/crowning-/coinvest.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit coinvest=${COMMIT} --url coinvest=${URL} ../coinvest/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coinvest=${COMMIT} --url coinvest=${URL} ../coinvest/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coinvest=${COMMIT} --url coinvest=${URL} ../coinvest/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the coinvest git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../coinvest/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/coinvest
SIGPATH=/some/root/path/coinvest-detached-sigs

./bin/gbuild --url coinvest=${BTCPATH},signature=${SIGPATH} ../coinvest/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coinvest-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coinvest-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/coinvest-osx-build.assert
```
...
[coinvest/gitian.sigs](https://github.com/coinvestpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
